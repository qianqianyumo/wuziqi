...
void GameWidget::showWinner(int winner)
{
    if(winner!=2)
    {
        ...
        QMessageBox::information(this," 游戏结束",tr("恭喜%1获胜").arg(playerName),QMessageBox::Ok);//显示一个信息图框
    }
    else
    {
...
{
    humanGameBtn->setEnabled(false);//设置成false按钮不可点击
    aiGameBtn->setEnabled(true);//按钮可以点击
...
void GameWidget::nextDropPiece(bool player)
{
    ...
    {
        QPoint pos = ai->searchGoodPos(boardWidget->getBoard());
